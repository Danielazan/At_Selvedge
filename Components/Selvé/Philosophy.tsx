...
// "craft" is intentionally excluded here — it no longer renders its own
// GLTF instance. That pillar docks the one persistent <G_cloth /> owned
// by Components/Scene/PersistentCloth.tsx instead (see PillarVisual).
const PILLAR_MODELS: Partial<Record<Pillar["visual"], React.ComponentType>> = {
  heritage: GlobeModel,
  innovation: InnoModel,
  luxury: GlobeModel,
}
...
function PillarVisual({ kind }: { kind: Pillar["visual"] }) {
  const boxClass =
    "relative h-[80px] w-[64px] shrink-0 overflow-hidden rounded-lg border border-white/[0.06] bg-[#0b0b0d] md:h-[92px] md:w-[74px]"
  const glowStyle: React.CSSProperties = {
    background: "radial-gradient(80px 90px at 22% 18%, rgba(228,197,103,0.22), transparent 62%)",
  }

  // The "African Craft" pillar is the Philosophy-side dock for the
  // single persistent G_cloth model that relays through to FourHouses'
  // ATELION card. No local Canvas/model here — PersistentCloth.tsx
  // reads this element's live screen rect every frame instead.
  if (kind === "craft") {
    return <ClothAnchor id="philosophy-craft" className={boxClass} style={glowStyle} />
  }

  const Model = PILLAR_MODELS[kind]
  if (!Model) return <div className={boxClass} style={glowStyle} />
