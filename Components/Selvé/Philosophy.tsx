...
/* ------------------------------------------------------------------ */
/* Pillar visuals — small auto-rotating 3D models.                     */
/* ------------------------------------------------------------------ */
/* Each pillar icon box is a fixed, fairly small target (64x80 /       */
/* 74x92 px), and the underlying GLTFs (global / g_cloth / inno) all   */
/* ship with arbitrary raw transforms (huge offsets, 0.001 scales,     */
/* etc — see SmallerModels.tsx), so we lean on the exact same          */
/* Bounds+Center pattern LandModel.tsx uses for its ModelThumb badges  */
/* to auto-fit the camera to whatever the model's real bounding box    */
/* is, then spin it continuously with OrbitControls' autoRotate. */

// Only 3 distinct 3D models exist (global, g_cloth, inno) for 4
// pillars, so the most thematically fitting one is reused twice:
// "global" -> Heritage (African-head model, reads as origin/roots)
// "inno"   -> Innovation (name match)
// "g_cloth"-> African Craft (gold dress, reads as textile/craft)
// "global" -> Global Luxury (name match, reused)
